...
export class UniversalSceneRenderer {
  ...
  static renderScene(scene, project = {}, assets = [], options = {}) {
    ...
    const replacements = {
      ...
      title: scene.settings?.titleText || scene.settings?.title || scene.name || 'Happy Birthday',
      message: scene.settings?.messageText || scene.settings?.textContent || scene.settings?.subtitleText || project?.recipient?.description || 'Wishing you all the happiness in the world!',
      customMessage: scene.settings?.messageText || scene.settings?.textContent || scene.settings?.subtitleText || project?.recipient?.description || 'Wishing you all the happiness in the world!',
      subtitle: scene.settings?.subtitleText || scene.settings?.subtitle || '',
      ...
      photo1: assets.find(a => a.id === scene.slots?.hero_image || a.id === scene.slots?.hero_photo || a.id === scene.slots?.fullscreen_image || a.id === scene.slots?.reveal_photo || a.id === scene.slots?.celebration_photo || a.id === scene.settings?.heroPhotoAssetId || a.id === scene.settings?.photoAssetId || a.type === 'image')?.renderUrl || scene.settings?.heroPhotoUrl || scene.settings?.photoUrl || 'https://images.unsplash.com/photo-1513151233558-d860c5398176?auto=format&fit=crop&w=1200&q=80',
      ...
      video1: assets.find(a => a.id === scene.slots?.main_video || a.id === scene.slots?.video || a.id === scene.settings?.videoAssetId || a.type === 'video')?.renderUrl || scene.settings?.videoUrl || '',
