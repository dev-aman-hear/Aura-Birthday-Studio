...
export class SceneTimelineView {
  ...
  render() {
    ...
    this.attachEvents(timeline);
    return timeline;
  }

  refreshTimeline(timeline) {
    const strip = timeline.querySelector('#timelineScrollStrip') || timeline;
    const scenes = [...(this.project.scenes || [])].sort((a, b) => a.order - b.order);
    strip.innerHTML = this.renderCardsHtml(scenes);
  }
  ...
  attachEvents(timeline) {
    timeline.addEventListener('click', async (e) => {
      ...
      if (btnDup) {
        e.stopPropagation();
        const clone = sceneRepository.duplicateScene(this.project, btnDup.dataset.sceneId);
        if (clone) {
